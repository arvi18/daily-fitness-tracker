...
export function buildPeriodProgress(logs, goals, start, end, periodLabel) {
  ...
  const metricDefs = [
    ...
    },
    {
      ...
      label: "Water",
      unit: "ml",
      current: avg("water_ml"),
      target: goals.water_ml_target,
      days_met: daysMet((log) => (log.water_ml ?? 0) >= goals.water_ml_target),
      met: daysLogged > 0 && daysMet((log) => (log.water_ml ?? 0) >= goals.water_ml_target) >= Math.ceil(daysLogged / 2),
    },
  ];
